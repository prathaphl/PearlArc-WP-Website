...
function neuros_customizer_dependancy() {
...
function neuros_customizer_repeater_uniqid(prefix, more_entropy) {
...
function neuros_customizer_repeater_refresh_social_icons(th) {
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + neuros_customizer_repeater_uniqid();
    ...
    neuros_customizer_repeater_refresh_general_control_values();
}

function neuros_customizer_repeater_refresh_general_control_values() {
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + neuros_customizer_repeater_uniqid();
            ...
            if ( title !== '' || icon_value !== '' || link !== '' ) {
                values.push({
                    ...
                    'link': link,
                    'title': neuros_escape_html(title),
                    'id': id
                });
            }
        });
...
function neuros_get_resolution_value($element) {
    var selected_values = {
...
function neuros_get_dimensions_value($element) {
    var selected_values = {
...
function neuros_get_font_value($element) {
    var selected_values = {
...
function neuros_ctype_digit(text) {
...
function neuros_escape_html(string) {
...
jQuery(document).ready(function($) {
    ...
    */
    $('.customize-control-resolution input[type="number"]').on('change', function() {
        neuros_get_resolution_value( $(this).parents('.customize-control-resolution') );
    });
    ...
    */
    $('.customize-control-dimensions select, .customize-control-dimensions input[type="number"]').on('change', function() {
        neuros_get_dimensions_value( $(this).parents('.customize-control-dimensions') );
    });

    /**
     * Google Font Custom Control
     */
    $('.neuros-google-fonts-list').on('change', function () {
        var elementWeightStyle = $(this).parent().parent().find('.neuros-google-font-styles');
        var elementSubset = $(this).parent().parent().find('.neuros-google-font-subset');
        var elementWeight = $(this).parent().parent().find('.neuros-google-font-weight');
        ...
        $.each(gflist[index].weights, function (val, text) {
            ...
            text = $.trim(text);
            if (optval === selectedWeight && neuros_ctype_digit(optval)) {
                elementWeightStyle.append(
                    $('<option></option>').val(optval).html(text).attr('selected', 'selected')
                ).val(optval).change();
            } else if (neuros_ctype_digit(optval)) {
                elementWeightStyle.append(
                    $('<option></option>').val(optval).html(text)
                );
            }
    ...
    $('.customize-control-google-font select, .customize-control-google-font input[type="number"]').on('change', function() {
        neuros_get_font_value( $(this).parents('.customize-control-google-font') );
    });
...
jQuery(document).ready(function() {
    ...
    theme_conrols.on('click', '.customizer-repeater-new-field', function () {
        ...
        var id = 'customizer-repeater-' + neuros_customizer_repeater_uniqid();
